/** NewPostTab component for AdminPage — handles image upload + wallet fields */
...
export function NewPostForm({ onDone }) {
  const [form, setForm] = useState({
    title: '', body: '', is_published: '1',
  ...
  async function handleSubmit(ev) {
    ...
    const { ok, data } = await api.upload('/blog', fd);
    setLoading(false);

    if (ok) {
      setAlert({ text: 'Post created!', type: 'success' });
      setTimeout(onDone, 1000);
    } else {
      setAlert({ text: data.message ?? 'Failed to create post.', type: 'error' });
    }
  }
          ...
          {imagePreview && (
            <div style={{marginTop:'0.5rem',border:'1px solid var(--border-dim)',position:'relative'}}>
              <img src={imagePreview} alt="preview"
                style={{width:'100%',maxHeight:'220px',objectFit:'contain',display:'block',filter:'saturate(0.85)'}} />
        ...
        <div style={{border:'1px solid var(--border-dim)',padding:'0.75rem',background:'var(--bg-panel)'}}>
          ...
          {form.wallet_enabled && (
            <div style={{marginTop:'0.75rem',display:'flex',flexDirection:'column',gap:'0.6rem'}}>
              ...
              <div className="field">
                <label className="label" htmlFor="eth-addr">ETH Address (ERC-20)</label>
                <input id="eth-addr" className={`input ${errors.eth_address ? 'is-error' : ''}`}
              ...
              <div className="field">
                <label className="label" htmlFor="btc-addr">BTC Address</label>
                <input id="btc-addr" className={`input ${errors.btc_address ? 'is-error' : ''}`}
